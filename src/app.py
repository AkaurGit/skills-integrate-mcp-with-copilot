...
Features persistent data storage via JSON file.
"""
...
try:
    ...
    logger.info(f"Successfully loaded {len(activities)} activities from storage")
...
def signup_for_activity(activity_name: str, email: str):
    ...
    try:
        ...
        logger.info(f"Student {email} signed up for {activity_name}")
    except Exception as e:
...
def unregister_from_activity(activity_name: str, email: str):
    ...
    try:
        save_activities(activities)
        logger.info(f"Student {email} unregistered from {activity_name}")
    except Exception as e:
        logger.error(f"Failed to save activities: {e}")
        raise HTTPException(status_code=500, detail="Failed to save unregistration")
